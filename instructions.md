...
>**Note** If you used the Cloudformaton template `DAT216-lab-instance.yml` for the prerequisites; then move to Section 3 (Create a database user account that uses an AWS authentication token). 

### Section 1. Enable IAM DB authentication on the RDS DB instance
...
### Section 2. Connect to an EC2 instance and install the MySQL server package
...
### Section 4. Create an IAM role allowing Amazon RDS access
...
### Section 5. Add an IAM policy that maps the database user to the IAM role
...
### Section 6. Attach the IAM role to the EC2 instance
...
### Section 7. Generate an AWS authentication token to identify the IAM role
...
### Section 8. Download the SSL root certificate file or certificate bundle file
...
### Section 9. Connect to the RDS DB instance using IAM role credentials and the authentication token
